...
object FirebaseRelay {
    ...
    // ========================================================================
    // REACTIONS (SYNCED BETWEEN BOTH USERS)
    // ========================================================================
    ...
    /**
     * Write a reaction to Firebase so the other participant sees it.
     * Path: /conversations/{conversationId}/reactions/{messageTimestamp}_{reactorUid}
     */
    suspend fun sendReaction(conversationId: String, messageTimestamp: Long, emoji: String) {
        val uid = getCurrentUid() ?: return
        val key = "${messageTimestamp}_${uid}"
        suspendCancellableCoroutine { cont ->
            val ref = database.reference
                .child("conversations")
                .child(conversationId)
    ...
    /**
     * Listen for reaction changes on a conversation.
     * Emits ReactionEvent for each new or changed reaction.
     */
    fun listenForReactions(conversationId: String): Flow<ReactionEvent> = callbackFlow {
        val ref = database.reference
            .child("conversations")
            .child(conversationId)
            .child("reactions")
        ...
        val listener = object : com.google.firebase.database.ChildEventListener {
            override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                parseReactionSnapshot(snapshot)?.let { trySend(it) }
            }
            override fun onChildChanged(snapshot: DataSnapshot, previousChildName: String?) {
                parseReactionSnapshot(snapshot)?.let { trySend(it) }
            }
            override fun onChildRemoved(snapshot: DataSnapshot) {
                // When removed, emit with empty emoji to clear
                val ts = snapshot.child("messageTimestamp").getValue(Long::class.java) ?: return
                val uid = snapshot.child("reactorUid").getValue(String::class.java) ?: return
                trySend(ReactionEvent(ts, "", uid))
            }
            override fun onChildMoved(snapshot: DataSnapshot, previousChildName: String?) {}
            override fun onCancelled(error: DatabaseError) {}
        }
    ...
    private fun parseReactionSnapshot(snapshot: DataSnapshot): ReactionEvent? {
        val ts = snapshot.child("messageTimestamp").getValue(Long::class.java) ?: return null
        val emoji = snapshot.child("emoji").getValue(String::class.java) ?: return null
        val uid = snapshot.child("reactorUid").getValue(String::class.java) ?: return null
        return ReactionEvent(ts, emoji, uid)
    }
