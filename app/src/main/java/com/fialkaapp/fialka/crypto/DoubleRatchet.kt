...
object DoubleRatchet {
    ...
    fun initializeAsInitiator(identitySharedSecret: ByteArray): InitialRatchetState {
        ...
        val rootKey = hkdfExpand(identitySharedSecret, "Fialka-DR-root".toByteArray())
        val sendChainKey = hkdfExpand(rootKey, "Fialka-DR-chain-init-send".toByteArray())
        val recvChainKey = hkdfExpand(rootKey, "Fialka-DR-chain-init-recv".toByteArray())
    ...
    fun initializeAsResponder(identitySharedSecret: ByteArray): InitialRatchetState {
        ...
        val recvChainKey = hkdfExpand(rootKey, "Fialka-DR-chain-init-send".toByteArray())
        val sendChainKey = hkdfExpand(rootKey, "Fialka-DR-chain-init-recv".toByteArray())
        identitySharedSecret.fill(0)
    ...
    fun pqRatchetStep(rootKeyBase64: String, pqSharedSecret: ByteArray): String {
        ...
        val newRootKey = hkdfExpand(combined, "Fialka-SPQR-pq-ratchet".toByteArray())
        rootKey.fill(0)
