...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onNewToken(token: String) {
        ...
        val prefs = getSharedPreferences("fialka_settings", MODE_PRIVATE)
        val pushEnabled = prefs.getBoolean("push_notifications_enabled", false)
        if (pushEnabled) {
            serviceScope.launch {
                try {
                    com.fialkaapp.fialka.data.remote.FirebaseRelay.storeFcmToken(token)
                } catch (_: Exception) { }
            }
        }
    }
    ...
    companion object {
        const val CHANNEL_ID = "fialka_messages"
